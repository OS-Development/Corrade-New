    ...
    internal class NucleusHTTPServer : HTTPServer
    {
        ...
        public readonly Timer CacheExpiryTimer = new Timer(((Expression<Action>) (() =>
            Nucleus.Value.Values.AsParallel()
                ...
                .Take(1)
                .ForAll(o => PurgeNucleus.BeginInvoke(null, null)
                                        ...
                                        default:
                                            lock (NucleusLock)
                                            {
                                                        ...
                                                        out file))
                                                {
                                                    ...
                                                    NucleusResponse.ContentType = IO.MimeType(file.Name);
                                                    break;
                                                }
                                                                ...
                                                                Encoding.UTF8.GetBytes(
                                                                    CSV.FromEnumerable(
                                                                        Nucleus.Value.Keys.AsParallel()
                                                                            .Select(o => o.Split('/').FirstOrDefault())
                                                                            .Where(o => !string.IsNullOrEmpty(o))
                                                                            .Distinct()))))
                                                    {
                        ...
                        using (var HTTPServerResponse = httpContext.Response)
                        {
                            try
                            {
                                using (var dataMemoryStream = new MemoryStream())
                                {
                                    ...
                                    {
                                        case false:
                                            // Generate a temporary group if no group was specified.
        ...
        private void PutFile(string type, string file, ref Stream dataMemoryStream)
        {
            // rewind
            dataMemoryStream.Seek(0, SeekOrigin.Begin);
            ...
            try
            {
                switch (file)
                {
                    case "Corrade.ini":
                        ...
                        {
                            dataMemoryStream.CopyTo(fileStream);
                        }
                        break;
                    default:
                        throw new HTTPException((int) HttpStatusCode.Forbidden);
        ...
        private void GetFile(string type, string file, HttpListenerResponse NucleusResponse,
        ...
        public class CoreFile
        {
            ...
            public TimeSpan CacheExpire { get; set; } = TimeSpan.FromSeconds(60);
