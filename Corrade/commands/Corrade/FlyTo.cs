...
namespace Corrade
{
    public partial class Corrade
    {
        ...
        {
            public static Action<Group, string, Dictionary<string, string>> flyto = (commandGroup, message, result) =>
            {
                ...
                if (!Vector3.TryParse(wasInput(
                    wasKeyValueGet(wasOutput(wasGetDescriptionFromEnumValue(ScriptKeys.POSITION)), message)),
                    out position))
                {
                ...
                if (!uint.TryParse(wasInput(
                    wasKeyValueGet(wasOutput(wasGetDescriptionFromEnumValue(ScriptKeys.DURATION)), message)),
                    out duration))
                {
                ...
                if (!int.TryParse(wasInput(
                    wasKeyValueGet(wasOutput(wasGetDescriptionFromEnumValue(ScriptKeys.AFFINITY)), message)),
                    out affinity))
                {
                    affinity = 2;
                }
                ...
                switch (bool.TryParse(wasInput(
                    wasKeyValueGet(wasOutput(wasGetDescriptionFromEnumValue(ScriptKeys.FLY)), message)), out fly))
                {
                    case true:
                        lock (ClientInstanceSelfLock)
                        {
